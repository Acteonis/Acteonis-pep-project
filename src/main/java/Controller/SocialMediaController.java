...
//|| || accountService.findByUsername(addedAccount.getUsername()).isPresent()
        if (account.getUsername().isEmpty() || account.getPassword().length() < 4 || accountService.findByUsername(account.getUsername()).isPresent()) {
            ctx.status(400);
            ctx.json("");
            //return;
        }*/
        ...
        if(accountService.findByUsername(addedAccount.getUsername()).isPresent() ){
            ctx.status(400);
            ctx.json("");
        }
        ctx.status(200);*/
...
If the login is not successful, the response status should be 401. (Unauthorized)*/
    ...
    private void handleLogin (Context ctx) throws Exception {
        ...
        Account account = ((ObjectMapper) mapper).readValue(ctx.body(), Account.class);
...
If the update of the message is not successful for any reason, the response status should be 400. (Client error)*/
    ...
    private void updateMessageHandler(Context ctx) throws IOException, SQLException {
        ObjectMapper mapper = new ObjectMapper();
        Message message = mapper.readValue(ctx.body(), Message.class);
        ...
        /*private void updateMessageByIDHandler (Context ctx) throws IOException, SQLException {
            ObjectMapper mapper = new ObjectMapper();
            Message message = mapper.readValue(ctx.body(), Message.class);
            int id = Integer.parseInt(ctx.pathParam("message_id"));
